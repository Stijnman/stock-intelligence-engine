...
st.title("Stock Intelligence Engine v2.8.0 — Insider Form 4 Clustering + Narrative Velocity Forecasting")
...
if refresh_interval > 0:
    st.info(
        ...
        "X narratives, velocity forecasts & **Insider Form 4 clusters**."
    )
    placeholder = st.empty()
    while True:
        with placeholder.container():
            report = analyze_watchlist(cfg, include_news=True, include_social=True, include_insider=True)
            df = pd.DataFrame(report["rows"])
            display_cols = [c for c in df.columns if c not in ("headlines", "transactions")]
            st.dataframe(df[display_cols], use_container_width=True)

            st.subheader("Signals · Buzz · X Narratives · Forecasts · Insider Clusters")
            for row in report["rows"]:
                ...
                st.write(
                    ...
                    f"Buzz {row.get('buzz_score',0)} | {narr}{forecast}{insider}{news_info}"
                )
