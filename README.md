...
* **v2.40.0 (2026-09-22)** : Fully implemented **TRACE Corporate-Bond Customer-Flow & Liquidity Shock Overlay** (`sie/trace_flow.py`). Wired through analyzer (`include_trace_flow`), CLI (`--no-trace-flow`), config (`trace_flow:`), Streamlit preferred columns (`trace_customer_flow`, `trace_liq_score`, `trace_dispersion`, `trace_boost`, `trace_reason`) and dedicated unit tests. Deterministic synthetic proxy; soft ±1 only. Removed from FUTURE-IMPROVEMENTS.md High Priority.
